 ...
 * @author alebenkov
 */
public class ZahtjeviUserBean implements Serializable {
    ...
    public ZahtjeviUserBean() {

    }

    public List<Dnevnik> getDnevnik() throws ParseException {
        DBOps db = new DBOps();
        dnevnik = db.ucitajZahtjeveKorisnika(request.getUserPrincipal().getName());
        if (request.getParameter("submit") != null) {
            if (request.getParameter("adresa") != "") {
                for (Iterator<Dnevnik> iterator = dnevnik.iterator(); iterator.hasNext();) {
                    if (!iterator.next().getNaredba().contains(request.getParameter("adresa"))) {
                        iterator.remove();
                    }
                }
            }
